...
2. browser_scriptname - returns the full browser name string, if available, otherwise returns ''
...
7. full_assoc - returns all relevant browser information in an associative array, same as above
   only with string indexes instead of numeric:
     ...
     'os' => $os_type,
     'os_number' => $os_number,
     'browser_scriptname_' => $browser_scriptname,
     'ua_type' => $ua_type,
...
function browser_detection( $which_test, $test_excludes='', $external_ua_string='' ) 
{
	...
	/*
	/*
	global $a_full_assoc_data, $a_mobile_data, $a_moz_data, $a_webkit_data, $b_dom_browser, $b_repeat, $b_safe_browser, $browser_scriptname, $browser_number, $browser_math_number, $browser_user_agent, $browser_working, $ie_version, $mobile_test, $moz_number, $moz_rv, $moz_rv_full, $moz_release_date, $moz_type, $os_number, $os_type, $true_ie_number, $ua_type, $webkit_type, $webkit_type_number;
	*/
	...
	*/
	if ( !$b_repeat )
	{
		...
		$run_time = '';
		$true_ie_number = '';
		$ua_type = '';// default to bot since you never know with bots
		$webkit_type = '';
		$webkit_type_number = '';
		...
		*/
		...
		$a_browser_types = array(// search engine spider bots:
			...
			/* Our own bot detection */
			array( 'adsbot', false, 'adsbot', 'bot' ), // AdsBot-Google
			array( 'mnogo', false, 'mnogo', 'bot' ), // mnogo
			array( 'teoma', false, 'teoma', 'bot' ), // teoma
			array( 'crawl', false, 'crawl', 'bot', '-' ), // any crawler
			array( ' bot', false, ' bot', 'bot', '-' ), // any bot
			array( 'bot ', false, 'bot ', 'bot', '-' ), // any bot
			array( 'bot/', false, 'bot/', 'bot', '-' ), // any bot
			array( '/bot', false, '/bot', 'bot', '-' ), // any bot
			array( 'bot-', false, 'bot-', 'bot', '-' ), // any bot
			array( '-bot', false, '-bot', 'bot', '-' ), // any bot
			array( 'spider', false, 'spider', 'bot', '-' ), // any spider
			array( 'java/1', false, 'java', 'bot' ), // java lib			
			...
			array( 'indy library', false, 'indy library', 'lib' ),
			array( 'wget', false, 'wget', 'lib' ),
		...
		*/
		...
		for ( $i = 0; $i < $i_count; $i++ )
		{
			...
			if ( strstr( $browser_user_agent, $browser_temp ) )
			{
				...
				*/
				$b_safe_browser = true;
				$browser_scriptname = $browser_temp;// text string to id browser from array
				...
				// if ( strstr( $browser_user_agent, "augustbot/" ) ) { var_dump($browser_user_agent." = ".$a_browser_types[$i][3]. " ({$i}) " . $browser_working); }
				switch ( $browser_working )
				{
					...
					case 'ie':
						...
						*/
						$browser_number = get_item_version( $browser_user_agent, $browser_scriptname, true, 'trident/' );
						...
						break;
					case 'op':
						$browser_number = get_item_version( $browser_user_agent, $browser_scriptname );
						...
						for ( $j = 0; $j < $j_count; $j++ )
						{
							if ( strstr( $browser_user_agent, $a_webkit_types[$j] ) )
							{
								...
								*/
								...
								{
									$browser_scriptname = 'epiphany';
								}
								else
								{
									$browser_scriptname = $a_webkit_types[$j];
								}
								break;
							}
						}
				...
				$browser_name = !empty($a_browser_types[$i][4]) ? $a_browser_types[$i][4] : $browser_scriptname;
		...
		if ( !$b_success )
		{
			...
			the browser_scriptname is '', you can always test for that
			*/
			// delete this part if you want an unknown browser returned
			$browser_scriptname = substr( $browser_user_agent, 0, strcspn( $browser_user_agent , '();') );
			// this extracts just the browser name from the string, if something usable was found
			if ( $browser_scriptname && preg_match( '/[^0-9][a-z]*-*\ *[a-z]*\ *[a-z]*/', $browser_scriptname, $a_unhandled_browser ) )
			{
				$browser_scriptname = $a_unhandled_browser[0];
				
				if ( $browser_scriptname == 'blackberry' )
				{
					get_set_count( 'set', 0 );
				}
				$browser_number = get_item_version( $browser_user_agent, $browser_scriptname );
			...
			{
				$browser_scriptname = 'NA';
				$browser_number = 'NA';
			}

			// then uncomment this part
			//$browser_scriptname = '';//deletes the last array item in case the browser was not a match
			
			$browser_name = $browser_scriptname;
		}
		...
		if ( $b_mobile_test )
		{
			...
			if ( $mobile_test ) {
				if($mobile_test == '') {
					...
					$a_mobile_data = ucwords($mobile_test);
				}
				if ($ua_type != 'bot') {
					$ua_type = 'mobile';
				} else {
	...
	*/
	...
	if ( !$a_full_assoc_data )
	{
		$a_full_assoc_data = array(
			...
			'os' => $os_type,
			'os_number' => $os_number,
			'browser_scriptname' => $browser_scriptname,
...
function get_os_data ( $pv_browser_string, $pv_browser_scriptname, $pv_version_number  )
{
		...
		if ( !is_array( $os_working_data ) && strstr( $pv_browser_string, $os_working_data ) && !strstr( $pv_browser_string, "linux" ) )
		{
			...
			switch ( $os_working_type )
			{
				...
				case 'mac':
					...
					*/
					elseif ( ( $pv_browser_scriptname == 'saf' ) || ( $pv_browser_scriptname == 'cam' ) ||
						( ( $pv_browser_scriptname == 'moz' ) && ( $pv_version_number >= 1.3 ) ) ||
						( ( $pv_browser_scriptname == 'ie' ) && ( $pv_version_number >= 5.2 ) ) )
					{
						$os_working_number = 10;
					}
					break;
...
function check_is_mobile( $pv_browser_user_agent )
{
	...
	*/
	...
	// browsers
	'avantgo', 'blazer', 'elaine', 'eudoraweb', 'iemobile',  'minimo', 'mobile safari', 'mobileexplorer', 'opera mobi', 'opera mini', 'netfront', 'opwv', 'polaris', 'semc-browser', 'up.browser', 'wms pie', 'xiino', 
...
function get_mobile_data( $pv_browser_user_agent )
{
	...
	$a_mobile_browser = array( 'applewebkit', 'avantgo', 'blazer', 'dolfin', 'elaine', 'eudoraweb', 'iemobile',  'minimo', 'mobile safari', 'mobileexplorer', 'opera mobi', 'opera mini', 'netfront', 'opwv', 'polaris', 'semc-browser', 'up.browser', 'webpro', 'wms pie', 'xiino' );
	/*
...
function get_useragent($useragent) {
	global $custom_bot_detection;
	$tmp_ua = browser_detection('full_assoc', '', $useragent);
	// var_dump($tmp_ua['mobile_data']);
	$new_ua = array(
		"agent_name" => "",
		"agent_version" => "",
		"agent_os" => "",
	...
	if($tmp_ua['ua_type'] == 'bro') {
		$operating_system = humanifyOS($tmp_ua);
		$new_ua['agent_os'] = $operating_system["os"];
		$new_ua['agent_os_version'] = $operating_system["os_version"];
		
		$new_ua['agent_engine'] = $tmp_ua['browser_working'];
		
		if(!empty($tmp_ua['ie_version'])) {
			// ie
			$new_ua['agent_name'] = "Internet Explorer";
			
			if(!empty($tmp_ua['true_ie_number'])) {
				$new_ua['agent_version'] = "{$tmp_ua['true_ie_number']} (Compatability View: IE {$tmp_ua['browser_number']})";
			} else {
				$new_ua['agent_version'] = $tmp_ua['browser_number'];
			}
		} else if(!empty($tmp_ua['moz_data'][0])) { // FireFox
			$real_version_no = "";
			$version_no = explode(".", $tmp_ua['moz_data'][1]);
			$real_version_no = $version_no[0];
			if(!empty($version_no[1])) {
				for($c = 0; $c < strlen($version_no[1]); $c++) {
					if(is_numeric($version_no[1][$c])) {
						$real_version_no .= ".".$version_no[1][$c];
					} else {
						break;
					}
				}
			...
			$new_ua['agent_name'] = ucwords(strtolower($tmp_ua['moz_data'][0]));
			$new_ua['agent_version'] = $real_version_no;
		} else if(!empty($tmp_ua['webkit_data'][0])) { // Chrome
			$real_version_no = "";
			$version_no = explode(".", $tmp_ua['webkit_data'][1]);
			$real_version_no = $version_no[0];
			
			$new_ua['agent_name'] = ucwords(strtolower($tmp_ua['webkit_data'][0]));
			$new_ua['agent_version'] = $real_version_no;
		} else if(!empty($tmp_ua['browser_name']) && $tmp_ua['browser_name'] == 'opera') { // Opera
			$real_version_no = "";
			$version_no = explode(".", $tmp_ua['webkit_data'][1]);
			$real_version_no = $version_no[0];
			
			$new_ua['agent_name'] = ucwords(strtolower($tmp_ua['browser_name']));
			$new_ua['agent_version'] = $real_version_no;
		} else if(!empty($tmp_ua['browser_name']) && $tmp_ua['browser_name'] == 'safari') { // Safari
			$new_ua['agent_name'] = ucwords(strtolower($tmp_ua['browser_name']));
		} else { // Something else
			$new_ua['agent_name'] = ucwords(strtolower($tmp_ua['browser_name']));
		}
	} elseif($tmp_ua['ua_type'] == 'mobile' || $tmp_ua['ua_type'] == 'mobilebot') {
		$new_ua['agent_is_mobile'] = 1;
		$new_ua['agent_name'] = !empty($tmp_ua['browser_name']) ? ucwords(strtolower($tmp_ua['browser_name'])) : "";
		$new_ua['agent_version'] = !empty($tmp_ua['browser_number']) ? $tmp_ua['browser_number'] : "";
		
		$tmp_os = humanifyOS($tmp_ua);
		$new_ua['agent_os'] = !empty($tmp_os['os']) ? ucwords(strtolower($tmp_os['os'])) : "";
		$new_ua['agent_os_version'] = !empty($tmp_os['os_version']) ? $tmp_os['os_version'] : "";
			
		$new_ua['agent_device'] = !empty($tmp_ua['mobile_data']) ? $tmp_ua['mobile_data'] : "";	
		if ($tmp_ua['ua_type'] == 'mobilebot') { 
			$new_ua["agent_is_bot"] = 1;
		}
	} elseif($tmp_ua['ua_type'] == 'bot') {
		$new_ua["agent_name"] = !empty($tmp_ua['browser_name']) ? ucwords(strtolower($tmp_ua['browser_name'])) : "";
		$new_ua["agent_version"] = !empty($tmp_ua['browser_version']) ? $tmp_ua['browser_version'] : "";
		$new_ua["agent_device"] = !empty($tmp_ua['browser_working']) ? $tmp_ua['browser_working'] : "";
		$new_ua["agent_is_bot"] = 1;
	} else {
		$new_ua["agent_name"] = !empty($tmp_ua['browser_name']) ? ucwords(strtolower($tmp_ua['browser_name'])) : "";
		$new_ua["agent_version"] = !empty($tmp_ua['browser_version']) ? $tmp_ua['browser_version'] : "";
		$new_ua["agent_device"] = !empty($tmp_ua['browser_working']) ? $tmp_ua['browser_working'] : "";
		$new_ua["agent_is_bot"] = 0;
	}
	
	$new_ua['agent_string'] = $useragent;
	// if(empty($new_ua['agent_name'])) { var_dump($new_ua['agent_string']); }
	if(empty($new_ua['agent_name']) || $new_ua['agent_name'] == '-') {
		$new_ua['agent_name'] = $new_ua['agent_string'];
	}
	
	foreach($new_ua as $agent_k => $agent_v) {
		$new_ua[$agent_k] = strip_tags(addslashes($agent_v));
	}
...
function humanifyOS($ua_array) {
	$os = $ua_array['os'];
	$os_version = $ua_array['os_number'];
	
	if($os == "lin") { // Linux
		$os = "Linux";
		$os_version = ucwords(strtolower($os_version));
	} else if($os == "nt" || $os == "win") { // Windows
		if($os == "nt") {
			switch($os_version) {
				case "6.1":
					$os_version = "7";
	...
	return array("os" => $os, "os_version" => $os_version);
}
...
// var_dump(browser_detection('full'));
?>