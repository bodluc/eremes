...
*  @version  Release: $Revision: 15681 $
...
class HomeSlider extends Module
{
	...
	public function __construct()
	{
		...
		$this->displayName = $this->l('Homeslider');
	...
	{
		/* Adds Module */
		if (parent::install() && $this->registerHook('displaySlider')  && $this->registerHook('displayHome') && $this->registerHook('actionShopDataDuplication'))
		{
	...
	private function _prepareHookDynamic()
	{
		$slider = array(
        ...
        //$product = ProductSale::getBestSales(6);
        //$product = Product::getNewProducts(6,2,5);
        
        for ($z=0;$z<5;$z++)
            $product[] = Product::getRandomSpecial(6);
        ...
        foreach ($product as &$p) {
            ...
            $dstImg = _PS_IMG_DIR_."slides/".$p['id_image'].".jpg";
            $p['path']="http://".$_SERVER['HTTP_HOST']._PS_IMG_."slides/".$p['id_image'].".jpg";
            if (!file_exists($dstImg)) {
                ImageManager::resize($srcImg,$dstImg,300,300);    
            }
        }
    ...
    public function _prepareHook() {
        ...
        $product[0]['path'] = '/img/slides/dls-r6a.jpg';
        $product[0]['link'] = 'http://www.audiorms.pl/car-audio/glosniki-samochodowe/p/DLS-R6A';
        
        $product[1]['path'] = '/img/slides/pc130.jpg'; 
        $product[1]['link'] = 'http://www.audiorms.pl/car-audio/glosniki-samochodowe/p/Focal-PC-130'; 
    ...
    public function hookDisplayHome() {
        ...
        return '<div id="slider-space" style="height:460px;"></div>';
    }
	...
	public function hookDisplaySlider()
	{
		...
		//$this->context->controller->addCSS($this->_path.'bx_styles2.css');
        
        $this->context->controller->addCSS($this->_path.'plus/css/style.css');
        $this->context->controller->addCSS($this->_path.'plus/css/plusslider.css');
        $this->context->controller->addJS($this->_path.'plus/js/jquery.plusslider.js');
		$this->context->controller->addJS($this->_path.'plus/js/plus.js');
        
		//return $this->display(__FILE__, 'homeslider.tpl');
        //return $this->display(__FILE__, 'pieacemaker.tpl');
