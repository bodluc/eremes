...
XML;
		...
		while ($product = Db::getInstance()->nextRow($resource))
		{
			...
			if (!isset($done[$id_product]['added']))
			{
				...
				// adds the product
				$tmpLink = $this->context->link->getProductLink((int)($product['id_product']), $product['link_rewrite'], $product['category'], $product['ean13'], (int)($product['id_lang']),null,null,true);
